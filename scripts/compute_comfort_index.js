...
async function computeAllComfortIndexes(date = null, windowWeeks = 4) {
  ...
  for (const unit of units) {
    try {
      const result = await computeUCIForUnit(unit._id, dateStr, windowWeeks, false);
      ...
      if (result) {
        ...
        if (successCount % 10 === 0) {
          console.log(`  Processed ${successCount}/${units.length} units...`);
        }
      } else {
        // 데이터가 없어서 계산 불가
        errorCount++;
      ...
      errorCount++;
      console.error(`  Error computing UCI for ${unit._id}:`, error.message);
    }
  }
